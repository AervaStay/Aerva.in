...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Basic sanity bounds — real latitudes/longitudes always fall in
    // these ranges, so anything outside them is clearly bad data (or a
    // tampered request) rather than a real location.
    const safeLat = latitude && !isNaN(Number(latitude)) && Math.abs(Number(latitude)) <= 90 ? Number(latitude) : null;
    const safeLng = longitude && !isNaN(Number(longitude)) && Math.abs(Number(longitude)) <= 180 ? Number(longitude) : null;

    // Only accept a cover photo that's genuinely one of this submission's
    // own photos — never trust it blindly, same defensive reasoning as
    // sanitizePhotoUrls above. Prevents a tampered request from setting
    // an arbitrary URL as the cover.
    const safeCoverUrl = (typeof coverPhotoUrl === 'string' && (safeExteriorUrls.includes(coverPhotoUrl) || safeInteriorUrls.includes(coverPhotoUrl)))
      ? coverPhotoUrl
      : null;
    ...
    if (existingDraft) {
      const updated = await sql`
        UPDATE listings SET
          ...
          latitude = ${safeLat}, longitude = ${safeLng}, formatted_address = ${formattedAddress || null},
      ...
      const inserted = await sql`
        INSERT INTO listings (
          ...
          latitude, longitude, formatted_address, cover_photo_url,
          ...
          ${safeLat}, ${safeLng}, ${formattedAddress || null}, ${safeCoverUrl},
